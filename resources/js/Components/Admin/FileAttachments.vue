...
<template>
    <div>
        ...
        <ul class="mb-2 space-y-1">
            <li v-for="f in existingItems" :key="'e' + f.id" class="flex items-center gap-2 rounded-lg border bg-slate-50 px-3 py-2 text-sm">
                <span>📎</span>
            ...
            <li v-for="(f, idx) in newFiles" :key="'n' + idx" class="flex items-center gap-2 rounded-lg border border-pjs-blue bg-blue-50 px-3 py-2 text-sm">
                ...
                <span class="text-xs text-slate-400">{{ humanSize(f.size) }}</span>
                <button type="button" class="text-red-500 hover:text-red-700" @click="removeNew(idx)">✕</button>
            </li>
        </ul>
        <button type="button" class="rounded-lg border border-dashed border-slate-200 px-4 py-2 text-sm text-slate-600 hover:border-pjs-blue hover:text-pjs-blue" @click="inputRef.click()">
