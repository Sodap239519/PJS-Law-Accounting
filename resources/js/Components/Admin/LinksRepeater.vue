        ...
        <div class="space-y-2">
            <div v-for="(row, idx) in rows" :key="idx" class="flex flex-col gap-2 sm:flex-row">
                <input
                    ...
                    class="w-full rounded-lg border-gray-300 text-sm sm:w-1/3"
                />
                <input
                    v-model="row.url"
                    type="url"
                ...
                />
                <button type="button" class="rounded-lg border border-red-200 px-3 text-red-600 hover:bg-red-50" @click="removeRow(idx)">✕</button>
            </div>
        </div>
        <button type="button" class="mt-2 rounded-lg border border-dashed border-gray-300 px-4 py-2 text-sm text-gray-600 hover:border-pjs-blue hover:text-pjs-blue" @click="addRow">
            + เพิ่มลิงก์
        </button>
    </div>
</template>
