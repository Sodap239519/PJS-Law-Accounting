...
const applyFilters = () => {
    router.get(route('admin.news.index'), { search: search.value, status: status.value }, { preserveState: true, replace: true });
};
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-3">
            <Link :href="route('admin.news.create')" class="rounded-lg bg-pjs-blue px-4 py-2 text-sm font-medium text-white hover:bg-pjs-blue-dark">
                + เพิ่มข่าว
            </Link>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="item in news.data" :key="item.id" class="hover:bg-slate-50">
                            <td class="px-4 py-2">
                            ...
                            <td class="px-4 py-2 text-right">
                                <Link :href="route('admin.news.edit', item.id)" class="text-pjs-blue hover:underline">แก้ไข</Link>
                                <button class="ml-3 text-red-500 hover:underline" @click="destroy(item.id)">ลบ</button>
                            </td>
                        </tr>
                        <tr v-if="!news.data.length">
                            <td colspan="7" class="px-4 py-10 text-center text-slate-400">ยังไม่มีข่าว</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="news.links && news.links.length > 3" class="mt-4 flex flex-wrap gap-1">
            <component
