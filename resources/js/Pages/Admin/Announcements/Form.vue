...
<template>
    ...
    <AdminLayout>
        <template #title>{{ isEdit ? 'แก้ไขประชาสัมพันธ์' : 'เพิ่มประชาสัมพันธ์' }}</template>
        ...
        <form class="mx-auto max-w-4xl space-y-6" @submit.prevent="submit">
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="space-y-5 lg:col-span-2">
                    ...
                    <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                        <FileAttachments :existing="announcement?.attachments || []" @update:files="form.attachments = $event" @update:deleted="deletedAttachments = $event" />
                    </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <Link :href="route('admin.announcements.index')" class="rounded-lg border px-4 py-2.5 text-center text-sm text-slate-600 hover:bg-slate-50">ยกเลิก</Link>
                    </div>
                </div>
            </div>
        </form>
