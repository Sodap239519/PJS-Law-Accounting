    ...
    <AdminLayout>
        ...
        <form class="mx-auto max-w-3xl space-y-6" @submit.prevent="submit">
            <div class="rounded-xl border bg-white p-5 shadow-sm">
                <label class="mb-1 block text-sm font-medium text-gray-700">ชื่อบริการ *</label>
                <input v-model="form.title" type="text" class="w-full rounded-lg border-gray-300" />
                <p v-if="form.errors.title" class="mt-1 text-sm text-red-600">{{ form.errors.title }}</p>

                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">ไอคอน (Bootstrap Icon)</label>
                        <input v-model="form.icon" type="text" placeholder="เช่น bi bi-briefcase" class="w-full rounded-lg border-gray-300" />
                    ...
                    <div>
                        ...
                        <input v-model="form.sort_order" type="number" class="w-full rounded-lg border-gray-300" />
                    </div>
                </div>

                <label class="mb-1 mt-4 block text-sm font-medium text-gray-700">รายละเอียด</label>
            ...
            <div class="flex gap-2">
                ...
                <Link :href="route('admin.services.index')" class="rounded-lg border px-5 py-2.5 text-center text-sm text-gray-600 hover:bg-gray-50">ยกเลิก</Link>
            </div>
        </form>
    </AdminLayout>
</template>
